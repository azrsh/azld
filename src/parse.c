...
ObjectFile *parse(const void *const head) {
  ...
  if (elf_header->e_shoff != 0) {
    ...
    const Elf64_Shdr *const shstr =
        &section_header_table[elf_header->e_shstrndx];
    ...
    {
      fprintf(stderr, "sym:\n");
      const Elf64_Sym *const symtab = head + symtab_section_header->sh_offset;
